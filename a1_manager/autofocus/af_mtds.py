...
class MMAutoFocus:
    ...
    def find_focus(self, searchRange: int=500)-> float:
        ...
        # Start autofocus
        afm_method.full_focus() # type: ignore
        return self.core.get_position() # type: ignore
    
    @staticmethod
    def _oughtaFocus(afm_method: Studio, searchRange: int=100, normalisation: str='FFTBandpass')-> Studio: 
        # Set properties
        ## Fixed prop.
        afm_method.set_property_value('ShowImages', 'No') # type: ignore ;determines whether images will be displayed in the Live Window during the autofocus routine.
        afm_method.set_property_value('Channel', '') # type: ignore ;allows you to select a channel preset to be used for autofocus – if no Channel is selected, then the current hardware settings will be used.
        afm_method.set_property_value('FFTLowerCutoff(%)', '2.5') # type: ignore ;
        afm_method.set_property_value('FFTUpperCutoff(%)', '14') # type: ignore ;
        afm_method.set_property_value('CropFactor', '1') # type: ignore ;The plugin will apply a camera ROI to the center of the field of view during autofocus, to reduce the time required to read out each image. A CropFactor of 1 means that the entire field will be used.
        afm_method.set_property_value('Tolerance_um', '0.2') # type: ignore ;Search will stop when the z position’s distance from maximum focus quality is estimated to be within the tolerance specified.
        afm_method.set_property_value('Exposure', '200') # type: ignore ;specifies how long the plugin will expose each image during the autofocus routine.
        ## User input prop.
        afm_method.set_property_value('SearchRange_um', str(searchRange)) # type: ignore ;OughtaFocus will search over z range specified, centered at the current z position.
        afm_method.set_property_value('Maximize', normalisation) # type: ignore ;determines what property of the autofocusing image series will be used to evaluate the quality of focus. You can choose Mean, Standard Deviation, or Edges (a function that looks at the variance of the first derivative). Experiment with different methods to find the best one for your sample.
...
class SqGradAutoFocus:
    ...
    def _determine_range_step(self, searchRange: int, step: int)-> list[float]:
        ...
        focus_device = self.a1_manager.core.get_property('Core','Focus') # type: ignore ; ZDrive, PFSOffset, MarZ
    ...
    def _capture_images_at_z(self, z_positions: list[float])-> list[np.ndarray]:
        ...
        for z in z_positions:
            ...
            self.a1_manager.core.set_position(z) # type: ignore
