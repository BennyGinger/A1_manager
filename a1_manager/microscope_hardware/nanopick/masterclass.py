...
class InjectionManager():
    ...
    def __init__(self,nanopick_dish: str, injection_device: str = None, needle_size = int, pressure = float | None = None, ): # type: ignore
        """
        ...
        if nanopick_dish = '96well':
            ...
            self.dish_calib: dict[str, dict[str, Any]]= json.load(f)
            self.keys = list(dish_calib.keys())
...
if __name__ == "__main__":
    ...
    # Automated stimulation protocol
     for well in list(master.keys):

         master.arm.to_home() # Lift up the head above the plate
         mt = master.dish_calib.get(well, {})
         position = StageCoord(xy=mt['center'])
         master.a1_manager.set_stage_position(position)
         sleep(1)
         
         # Injection of ligands
         master.device.injecting(inject_vol_ul=10, mixing_cycles=2)
         master.arm.to_liquid()
         master.arm.to_home()
         sleep(1)
